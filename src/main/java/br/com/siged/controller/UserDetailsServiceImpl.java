...
import br.com.siged.util.Ambiente;

@Component("userDetailsService")
public class UserDetailsServiceImpl implements UserDetailsService{
	
	private @Value("${siged.ambiente}") Ambiente ambiente;
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException, DataAccessException {
		...
		if (usuarioSCA != null && usuarioSCA.isAtivo() && participanteDao.findServidorByCpf(usuarioSCA.getCpf()) != null) {
			...
			System.out.println(ambiente + ": " + authorities);
			
			Usuario usuario = new Usuario(usuarioSCA);
			...
			if (usuarioSIGED != null && usuarioSIGED.isEnabled()) {				
				...
				if(usuarioSIGED.getTipo() == 1){
				...
				} else if (usuarioSIGED.getTipo() == 2) {
					
					authorities.add(new GrantedAuthorityImpl("ROLE_ADMINISTRADOR"));
					authorities.add(new GrantedAuthorityImpl("ROLE_RESET_PASSWORD"));
					...
					if(usuarioSIGED.isForcarAtualizacaoEmail()){
						authorities.add(new GrantedAuthorityImpl("ROLE_UPDATE_EMAIL_REQUIRED"));
					}
				...
				usuarioSIGED.setAuthorities(authorities);
