...
<script type="text/javascript">
	...
	<div class="body">
		...
		<c:if test="${modalidade != null}">
			<h1>Eventos Realizados - ${modalidade.descricao}</h1>			
		</c:if>
		<c:if test="${modalidade == null}">
			<h1>Eventos Realizados</h1>			
		</c:if>

		<c:if test="${fn:length(eventos) == 0 and mostrarMensagem}">
			<c:if test="${modalidade == null}">
				<div class="message">Não houve eventos realizados no período informado.</div>
			</c:if>
			<c:if test="${modalidade != null && modalidade.id == 1}">
				<div class="message">Não houve eventos presenciais realizados no período informado.</div>
			</c:if>
			<c:if test="${modalidade != null && modalidade.id == 2}">
				<div class="message">Não houve eventos a distância (EAD) realizados no período informado.</div>
			</c:if>
		</c:if>
		...
		<c:url var="url" value="/evento/searcheventos" />

		<div class="filter">
			<form:form action="${url}" method="GET" modelAttribute="eventofiltro">
				<form:hidden path="statusEvento" value="realizados" />
				<table>
					<tbody>
						<tr>
						...
						<tr style="height: 50px;">
							<td style="width: 16%;"><label for="realizacao">Período de Realização</label></td>
							<td class="value"><form:input cssStyle="width:90px"
									id="realizacao1" maxlength="255" path="realizacao1" size="30"
									alt="date" /> a <form:input cssStyle="width:90px"
									id="realizacao2" maxlength="255" path="realizacao2" size="30"
									alt="date" /> <form:hidden path="modalidadeId" /></td>
						</tr>
						<tr>
		...
		<c:forEach items="${eventos}" var="evento">
			...
			<h2>
				${evento.tipoId} ${evento.titulo} -	<fmt:formatDate pattern="dd/MM/yyyy" value="${evento.dataInicioPrevisto}" />
				a
							...
							<li id="avalia">								
								...
								<sec:authorize ifAnyGranted="ROLE_ALUNO,ROLE_CHEFE">														
									<c:if test="${evento.desempenho == 'S' && evento.id != 481}">
										<!-- BOTÃO DESEMPENHO -->
										<a href="<c:url value="/evento/desempenhos/${evento.id}/?n=${random}" />" title="Desempenhos" onclick="Modalbox.show(this.href, {title: this.title, width: 810, height: 500}); return false;">
											<img src="<c:url value="/static/images/icon_desempenho.png"/>" id="icon_desempenho" />
										</a>
									</c:if>																																															
								</sec:authorize>						
								...
								<sec:authorize ifAnyGranted="ROLE_ADMINISTRADOR,ROLE_ADMINISTRADORCONS">								
									...
									<c:if test="${fn:length(evento.gastoList) > 0}">								
										<!-- BOTÃO DE GASTOS -->
										<a href="<c:url value="/evento/gastos/${evento.id}/?n=${random}"/>" title="Gastos" onclick="Modalbox.show(this.href, {title: this.title, width: 810, height: 500}); return false;">
											<img src="<c:url value="/static/images/icon_gastos.png"/>" id="icon_avaliacao" />
										</a>
									</c:if>
									<c:if test="${evento.eventoComParticipante}">	
										<!-- BOTÃO DE PARTICIPANTES -->
										<a href="<c:url value="/evento/participantes/${evento.id}/?n=${random}"/>" title="Participantes" onclick="Modalbox.show(this.href, {title: this.title, width: 810, height: 500}); return false;">
											<img src="<c:url value="/static/images/icon_participantes.png"/>" id="icon_participantes" />
										</a>
									</c:if>								
								</sec:authorize>								
							</li>
			...
			$('.pre-inscricao--js-hook').click(function(event) {
				...
				jQuery.ajax({
					...
					error : function(error) {
						alert("Erro ao verificar possibilidade de inscrição: " + error.response);
					},
					complete : function() {
						loading.stop();
					}
